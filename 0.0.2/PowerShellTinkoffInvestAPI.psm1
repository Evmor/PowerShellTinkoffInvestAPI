...
.EXAMPLE
    Send-TIABondBy -IdType figi -Id BBG004730JJ5
.EXAMPLE
    Send-TIABondBy -IdType ticker -ClassCode TQBR -Id RU000A0JX0J2
.PARAMETER IdType
    Тип идентификатора инструмента. Возможные значения: figi, ticker, uid, position_uid.
.PARAMETER ClassCode
...
.PARAMETER from
    Начало периода в ISO 8601.
.PARAMETER to
    Окончание периода в ISO 8601.
.PARAMETER interval
    Интервал свечей.
.PARAMETER instrumentId
    Идентификатор инструмента.
.EXAMPLE
    Send-TIAGetCandles -figi 'BBG004730JJ5' -from $(Get-Date).AddMonths(-2) -to $(Get-Date) -Interval CANDLE_INTERVAL_HOUR
#>
function Send-TIAGetCandles {
    ...
    process {
        try {
            ...
            Write-Error "Ошибка при отправке запроса: $($Error[0].Message)"
        }
    }
    end {}
}
...
.DESCRIPTION
    Функция загружает исторические данные в формате .zip
#>
function Send-TIAGetHistory {
    [CmdletBinding()]
    param (
        # Год загрузки исторических данных
        [Parameter(Mandatory=$true)]
        [string]
        $Year,
        # figi инструмента
        ...
        [string]
        $figi,
        # Каталог для сохранения загруженных архивов исторических данных.
        [Parameter(Mandatory=$true)]
        [string]
        $OutputCatalog
    )
    begin {}
    process {
        $Uri = "https://invest-public-api.tinkoff.ru/history-data?figi=BBG00QKJSX05&year=2022"
        $Headers = @{
            "Authorization" = "Bearer $token"
        }
        
        $OutFile = "$($OutputCatalog)/$($figi)_$($Year).zip"
